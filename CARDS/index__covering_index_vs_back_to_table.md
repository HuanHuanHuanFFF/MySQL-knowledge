...
**Point**: 二级索引查询通常先在索引叶子里按条件定位；如果叶子里已经有这次查询要的全部列，就可以直接返回；如果还缺列，就要拿叶子里的主键值再去聚簇索引取整行，这个过程就叫回表。索引里就能答完查询时，这种情况就叫覆盖索引。

**Why**: 二级索引本来就不直接存整行，所以数据库要先看“索引里够不够回答这条查询”。够就直接返回，不够就再去主键索引取整行，这也是覆盖索引通常比回表更省的原因。
